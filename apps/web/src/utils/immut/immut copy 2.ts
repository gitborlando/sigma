...
export default class Immut<T extends AnyObject = AnyObject> {
  ...
  private _track = (patch: ImmutPatch) => {
    ...
    patch.keys.forEach((key, i) => {
      if (i !== patch.keys.length - 1) {
        if (!curMap[key]) curMap[key] = {}
        curMap = curMap[key]
      } else {
        curMap[key] = { patch: patch, __isPatch: true }
      }
    })
  }
  ...
  private traverse = (object: any, changeMap: any) => {
    ...
    for (const key in changeMap) {
      if ('__isPatch' in changeMap[key]) {
        const patch = changeMap[key].patch as ImmutPatch
        switch (patch.type) {
          case 'add':
            if (Array.isArray(object)) {
              const index = patch.keys[patch.keys.length - 1] as number
              object.splice(index, 0, patch.value)
            } else {
