import { macroMatch, objKeys } from '@gitborlando/utils'
  ...
  setupActiveGeometry(selectedNodes: V1.Node[]) {
    ...
    selectedNodes.forEach((node, i) => {
      this.activeKeys.forEach((key) => {
        if (i === 0) this.activeGeometry[key] = this.getNodeGeometryValue(node, key)
        else if (this.activeGeometry[key] !== this.getNodeGeometryValue(node, key))
          T<any>(this.activeGeometry)[key] = MULTI_VALUE
      })
    })
  }

  getNodeGeometryValue(node: V1.Node, key: keyof AllGeometry) {
    if (macroMatch`"x"|"y"|"width"|"height"|"rotation"`(key)) {
